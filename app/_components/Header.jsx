...
const Header = () => {
  ...
  return (
    <div className='flex items-center justify-between p-4 shadow-md'>
        ...
        <ul className={open? 'flex flex-col absolute top-20 gap-6 justify-center items-center ml-[-2.8%] w-[100%] z-40 md:hidden text-2xl font-bold bg-blue-100 text-blue-950 bg-opacity-80 py-5 rounded-lg':'gap-8 hidden md:flex' } >
           {Menu.map((item,index)=>(
            ...
            <ul onMouseOver={()=>setUserOpen(true)} onMouseOut={()=>setUserOpen(false)} className={userOpen?'shadow-lg z-50 absolute pl-4 pr-16 right-4 top-[11.4%] bg-red-300 rounded-sm':'hidden'}>
                <Link href='#'><li className='py-2 cursor-pointer hover:scale-105 hover:text-primary hover:font-bold transition-all ease-in-out'>Dashboard </li></Link>
